...
function deployCC() {
  if [ $ORG_USING -eq -1 ]; then
    scripts/deployCC.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
  else
    scripts/deployCC_org${PEER_NUM}.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE $ORG_USING
  fi
...
function networkDown() {
  ...
  echo $ORG_USING
  for ((i=3; i<=${ORG_USING}; i++))
  do
    COMPOSE_FILE_ORGX=addOrg/docker/docker-compose-org${i}.yaml
    docker-compose -f $COMPOSE_FILE_ORGX down --volumes --remove-orphans
  done
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    for ((i=3; i<=${ORG_USING}; i++))
    do
        docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf addOrg/fabric-ca/org'${i}'/msp addOrg/fabric-ca/org'${i}'/tls-cert.pem addOrg/fabric-ca/org'${i}'/ca-cert.pem addOrg/fabric-ca/org'${i}'/IssuerPublicKey addOrg/fabric-ca/org'${i}'/IssuerRevocationPublicKey addOrg/fabric-ca/org'${i}'/fabric-ca-server.db'
    done
    #docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf addOrg/fabric-ca/org3/msp addOrg/fabric-ca/org3/tls-cert.pem addOrg/fabric-ca/org3/ca-cert.pem addOrg/fabric-ca/org3/IssuerPublicKey addOrg/fabric-ca/org3/IssuerRevocationPublicKey addOrg/fabric-ca/org3/fabric-ca-server.db'
...
COMPOSE_FILE_ORG3=addOrg/docker/docker-compose-org3.yaml
#
